...
public class solution {
    ...
    public static void main(String[] args) {
        ...
        for (int T = 0; T < t; T++) {
            ...
            double distance = Math.sqrt(Math.pow(cX[i] - cX[j], 2) + Math.pow(cY[i] - cY[j], 2));
          
            double sumR1 = cR[i] + cR[j];
            double sumR2 = cR[i] - cR[j];
            double sumR3 = cR[j] - cR[i];
            

            if (distance > sumR1 || distance < sumR2) {
                System.out.printf("Circle %d does not intersect Circle %d", i, j);
            } else if (distance < sumR3) {
